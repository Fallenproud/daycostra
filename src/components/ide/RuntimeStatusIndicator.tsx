...
export function RuntimeStatusIndicator() {
  ...
  const readyChecks = useMemo(
    () => status.checks.filter((item) => item.ready).length,
    [status.checks],
  );
  ...
  const presentation = useMemo(() => {
    ...
    if (status.connected) {
      ...
      return {
        label: status.executionReady
          ? "Governed runtime ready"
          : "Runtime connected · authority locked",
  ...
  return (
    <div className="fixed right-[210px] top-[22px] z-[125] hidden lg:block">
      ...
      >
        ...
        <span
          className={cn("text-[9px] font-semibold uppercase tracking-[0.11em]", presentation.tone)}
        >
          {presentation.label}
        </span>
        <ChevronDown
        ...
        <div className="absolute right-0 top-[calc(100%+8px)] w-[380px] rounded-2xl border border-white/[0.09] bg-[#111217]/98 p-4 shadow-2xl backdrop-blur-xl">
          <div className="flex items-start justify-between gap-3">
            ...
            <button
              ...
              className="grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-white/[0.07] text-zinc-600 hover:text-zinc-200"
            >
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            ...
            <StatusCell
              label="Authority S6"
              value={status.authority.configured ? "Configured" : "Not configured"}
              good={status.authority.configured}
            />
            <StatusCell
              label="Execution"
              value={status.executionReady ? "Authorized" : "Locked"}
              good={status.executionReady}
            />
            <StatusCell label="Mode" value={status.executionMode} />
            <StatusCell
              label="Runtime checks"
              value={totalChecks ? `${readyChecks}/${totalChecks} checks` : "No probe data"}
              good={Boolean(totalChecks && readyChecks === totalChecks)}
            />
            <StatusCell
              label="Authority proof"
              value={status.authority.connected ? "Proven" : "Pending mission"}
              good={status.authority.connected}
            />
          </div>

          <div className="mt-3 rounded-xl border border-white/[0.06] bg-black/25 px-3 py-2.5">
            <div className="text-[8px] font-semibold uppercase tracking-[0.12em] text-zinc-700">
              Authority boundary
            </div>
          ...
          <button
            ...
            className="mt-3 inline-flex items-center gap-2 rounded-lg border border-white/[0.07] px-3 py-2 text-[9px] font-semibold uppercase tracking-[0.1em] text-zinc-400 hover:bg-white/[0.04] hover:text-zinc-200"
          >
...
function StatusCell({
  ...
  return (
    <div className="rounded-xl border border-white/[0.06] bg-black/25 p-3">
      ...
      <div
        className={cn(
          "mt-1.5 truncate text-[10px] font-medium capitalize text-zinc-400",
