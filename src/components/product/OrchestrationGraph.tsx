...
export function OrchestrationGraph({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("dc-graph-shell", className)}>
      ...
      <svg viewBox="0 0 660 180" role="img" aria-labelledby={titleId}>
        <title id={titleId}>
        ...
        <defs>
          ...
          <radialGradient id={nodeId} cx="45%" cy="35%" r="70%">
        ...
        <g className="dc-graph-edges">
          {edges.map(([from, to]) => {
            ...
            return <path key={`${from}-${to}`} d={d} fill="none" stroke={`url(#${edgeId})`} strokeWidth="1.5" />;
          })}
        </g>

        {nodes.map((node) => (
          <g key={node.id} transform={`translate(${node.x}, ${node.y})`} className="dc-graph-node">
            <circle r="28" fill={`url(#${nodeId})`} stroke="rgba(165,167,173,.45)" />
